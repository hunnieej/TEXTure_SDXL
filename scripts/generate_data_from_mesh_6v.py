...
def main(cfg: RunConfig):
    ...
    global_ind = 0
    while all([c == 5 for c in count_per_view.values()]): #default 256
        dataloader = ViewsDataset(render_cfg, device=device, size=4, random_views=False).dataloader()
        ...
        for i, data in enumerate(dataloader):
            ...
            if count_per_view[dir] == 5:
                # Break if all are above 32
                if all([c == 5 for c in count_per_view.values()]):
                    break
                continue
