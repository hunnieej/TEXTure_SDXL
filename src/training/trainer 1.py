...
class TEXTure:
    ...
    def calc_text_embeddings(self) -> Union[torch.Tensor, List[torch.Tensor]]: # Text embeddings -> text
    ...
    def paint_viewpoint(self, data: Dict[str, Any]):
        ...
        self.save_gray_image(depth_render, 'depth_gray')
        ...
        self.save_gray_image(update_mask, 'update_mask')
        self.save_gray_image(generate_mask, 'genrate_mask')
        self.save_gray_image(refine_mask, 'refine_mask')
        self.log_train_image(rgb_render * (1 - update_mask), name='검정색 영역이 update_mask')
        ...
        self.log_train_image(cropped_rgb_render, name='두번째 렌더링 이미지에서 crop된 이미지')
        ...
        cropped_rgb_output, steps_vis = self.diffusion.img2img_step(text_z, cropped_rgb_render.detach(),
                                                                    ...
                                                                    all_mask=all_mask, generate_mask=cropped_generate_mask, paint_step = self.paint_step)
        ...
        cropped_obj_mask = object_mask[:, :, min_h:max_h, min_w:max_w]
        full_output = rgb_render.clone()
        full_output[:, :, min_h:max_h, min_w:max_w] = (
            full_output[:, :, min_h:max_h, min_w:max_w] * (1 - cropped_obj_mask)
            + cropped_rgb_output * cropped_obj_mask
        )
        self.log_train_image(full_output, name='full_output')
        rgb_output = full_output
    ...
    def save_gray_image(self, image, name):
        image_np = (image[0, 0].cpu().numpy() * 255).astype(np.uint8)
        Image.fromarray(image_np).save(self.train_renders_path / f'{self.paint_step:04d}_{name}.png')
