...
from transformers import CLIPTextModel, CLIPTokenizer, logging, AutoTokenizer, PretrainedConfig, CLIPTextModelWithProjection
...
class StableDiffusion_inpaintXL(nn.Module):
    def __init__(self, device, model_name='CompVis/stable-diffusion-v1-4', concept_name=None, concept_path=None,
        ...
        inpaint_model_name = "diffusers/stable-diffusion-xl-1.0-inpainting-0.1"

        logger.info(f'loading stable diffusion with {model_name}...')
        logger.info(f'loading SDXL with {inpaint_model_name}...')
        ...
        if self.use_inpaint:
            self.inpaint_vae = AutoencoderKL.from_pretrained(inpaint_model_name, subfolder="vae", use_auth_token=self.token).to(self.device)
        ...
        if self.use_inpaint:
            self.inpaint_tokenizer = AutoTokenizer.from_pretrained(
                inpaint_model_name,
                subfolder="tokenizer",
                revision=None,
                use_fast=False,
            )
            self.inpaint_tokenizer_2 = AutoTokenizer.from_pretrained(
                inpaint_model_name,
                subfolder="tokenizer_2",
                revision=None,
                use_fast=False,
            )
            #self.inpaint_tokenizer = CLIPTokenizer.from_pretrained("diffusers/stable-diffusion-xl-1.0-inpainting-0.1", subfolder='tokenizer', use_auth_token=self.token)
            #self.inpaint_tokenizer_2 = CLIPTokenizer.from_pretrained("diffusers/stable-diffusion-xl-1.0-inpainting-0.1", subfolder='tokenizer_2', use_auth_token=self.token)
            self.inpaint_text_encoder = CLIPTextModelWithProjection.from_pretrained(inpaint_model_name, subfolder='text_encoder',
                                                            use_auth_token=self.token).to(self.device)
            self.inpaint_text_encoder_2 = CLIPTextModelWithProjection.from_pretrained(inpaint_model_name, subfolder='text_encoder_2',
                                                            use_auth_token=self.token).to(self.device)
        ...
        if self.use_inpaint:
            self.inpaint_unet = UNet2DConditionModel.from_pretrained(inpaint_model_name,
        ...
        self.scheduler = PNDMScheduler(beta_start=0.00085, beta_end=0.012, beta_schedule="scaled_linear",
                                       ...
                                       skip_prk_steps=True)
        if self.use_inpaint:
            self.inpaint_scheduler =  DDPMScheduler.from_pretrained(inpaint_model_name, subfolder="scheduler")
    ...
    def get_text_embeds(self, prompt, negative_prompt=None):
        ...
        # Inpaint용 임베딩 생성
        if self.use_inpaint:
            text_encoder = [self.inpaint_text_encoder, self.inpaint_text_encoder_2]
            inpaint_tokenizer = [self.inpaint_tokenizer, self.inpaint_tokenizer_2]
            prompt_embeds_list = []
            for tokenzier, text_encoder in zip(inpaint_tokenizer, text_encoder):
                with torch.no_grad():
                    text_inputs = tokenzier(
                    prompt,
                    padding="max_length",
                    max_length=tokenzier.model_max_length,
                    truncation=True,
                    return_tensors="pt",
                    )
            ...
            for tokenzier, text_encoder in zip(inpaint_tokenizer, text_encoder):
                with torch.no_grad():
                    negative_text_inputs = tokenzier(
                    negative_prompt,
                    padding="max_length",
                    max_length=tokenzier.model_max_length,
                    truncation=True,
                    ...
                    negative_prompt_embeds = text_encoder(
                        negative_text_input_ids.to(text_encoder.device),
                        output_hidden_states=True,
                        return_dict=False,
                    )
            ...
            negative_prompt_embeds = torch.concat(negative_prompt_embeds_list, dim=-1) # ([1, 77, 2048])
            negative_pooled_prompt_embeds = negative_pooled_prompt_embeds.view(bs_embed, -1) # ([1, 1280])

        prompt_embeds = torch.cat([negative_prompt_embeds, prompt_embeds])
        pooled_prompt_embeds = torch.cat([negative_pooled_prompt_embeds, pooled_prompt_embeds])

        return text_embeddings, (prompt_embeds, pooled_prompt_embeds)
    ...
    def img2img_step(self, text_embeddings, inputs, depth_mask, guidance_scale=100, strength=0.5,
        ...
        def sample(latents, depth_mask, strength, num_inference_steps, update_mask=None, check_mask=None,
                   masked_latents=None):
            text_embeds, (inpaint_text_embeddings, pooled_inpaint_embeds) = text_embeddings
            ...
            if latents is None:
                ...
                latents = torch.randn(
                    (
                        text_embeds.shape[0] // 2, self.unet.in_channels - 1, depth_mask.shape[2],
                        depth_mask.shape[3]),
                    device=self.device)
            ...
            with torch.autocast('cuda'):
                for i, t in tqdm(enumerate(timesteps)):
                    ...
                    if not is_inpaint_range and mask_constraints_iters:
                        if update_mask is not None:
                            noised_truth = self.inpaint_scheduler.add_noise(gt_latents, noise, t)
                    ...
                    if is_inpaint_iter:
                        latent_model_input = self.inpaint_scheduler.scale_model_input(latent_model_input,
                    ...
                    if is_inpaint_iter:
                        ...
                        # Augmentation 진행 안한다는 가정정
                        inpaint_original_sizes = [(1024, 1024)] * inpaint_text_embeddings.shape[0]  # 배치 크기만큼 반복
                        crop_top_lefts = [(0, 0)] * inpaint_text_embeddings.shape[0]       # 자르기 좌표 기본값 (0, 0)

                        # add_time_ids 생성
                        def compute_time_ids(original_size, crops_coords_top_left, target_size=(1024, 1024), device='cuda', dtype=torch.float32):
                            add_time_ids = list(original_size + crops_coords_top_left + target_size)
                            add_time_ids = torch.tensor([add_time_ids], device=device, dtype=dtype)
                            return add_time_ids
                        # add_time_ids 계산
                        add_time_ids = torch.cat([
                            compute_time_ids(s, c, device=self.device, dtype=inpaint_text_embeddings.dtype) 
                            for s, c in zip(inpaint_original_sizes, crop_top_lefts)
                        ])
                        
                        added_cond_kwargs = {
                            "text_embeds": pooled_inpaint_embeds, # [2, 1280]
                            "time_ids": add_time_ids  # [2,6]
                        }

                        with torch.no_grad():
                            noise_pred_inpaint = \
                                self.inpaint_unet(latent_model_input_inpaint, t, encoder_hidden_states=inpaint_text_embeddings, added_cond_kwargs=added_cond_kwargs)[
                                    'sample']
                        ...
                        # predict the noise residual
                        with torch.no_grad():
                            noise_pred = self.unet(latent_model_input_depth, t, encoder_hidden_states=text_embeds)[
                                'sample']
                    ...
                    if intermediate_vis:
                        if is_inpaint_iter:
                            vis_alpha_t = torch.sqrt(self.inpaint_scheduler.alphas_cumprod)
                            vis_sigma_t = torch.sqrt(1 - self.inpaint_scheduler.alphas_cumprod)
                        else:
                    ...
                    if is_inpaint_iter:
                        latents = self.inpaint_scheduler.step(noise_pred, t, latents)['prev_sample']
                    else:
    ...
    def produce_latents(self, text_embeddings_tuple, depth_mask, height=512, width=512, num_inference_steps=50,
        ...
        if latents is None:
            ...
            latents = torch.randn((text_embeds.shape[0] // 2, self.unet.in_channels - 1, height // 8, width // 8),
                                device=self.device)
            timesteps = self.scheduler.timesteps
        else:
            # Strength has meaning only when latents are given
    ...
    def prompt_to_img(self, prompts, depth_mask, height=512, width=512, num_inference_steps=50, guidance_scale=7.5,
        ...
        latents = self.produce_latents((text_embeds, inpaint_text_embeds), depth_mask=depth_mask, height=height, width=width, latents=latents,
